...
function runDetection() {
	model.detect(video).then((predictions) => {
		...
		if (predictions[0]) {
			...
			// console.log(predictions[0].bbox[1] + predictions[0].bbox[2] / 2)
			let midvalY = predictions[0].bbox[1] + predictions[0].bbox[3]
			// let midvalY2 = predictions[0].bbox[1] + predictions[0].bbox[3]
...
const animate = () => {
  ...
  ------------------------------*/
	...
	var vector = new THREE.Vector3(mappedVal, 0.3, 0.5)
	vector.unproject(camera)
	var dir = vector.sub(camera.position).normalize()
	var distance = -camera.position.z / dir.z
	var pos = camera.position.clone().add(dir.multiplyScalar(distance))
