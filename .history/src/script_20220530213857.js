...
class Sketch {
	...
	addAround() {
		...
		this.geometry1 = new THREE.SphereBufferGeometry(21.0, 30, 30)

		this.sunAround = new THREE.Mesh(this.geometry1, this.materialAround)
		scene.add(this.sunAround)
	}
    ...
    Cube Render
    ------------------------------*/
		this.cubeCamera1.update(renderer, this.scene1)
		...
		this.sun.rotation.z -= 0.0009
		// this.sunAround.rotation.x += 0.003
		// this.sunAround.rotation.y -= 0.003
		// this.sunAround.rotation.z -= 0.003
...
class Mover {
	...
	update() {
		this.material.uniforms.uTime.value = 0.00025 * (Date.now() - start)
...
const attract = (mover, attractor, amass, mmass) => {
	...
	let b = new THREE.Vector3(
		mover.mesh.position.x,
		mover.mesh.position.y,
		mover.mesh.position.z
	)
	let force = a.sub(b)
  ...
  ------------------------------*/
	...
	track.forEach((tracked) => {
		if (tracked.label == 'closed') {
			attract(test, sun.sun, 1, 1)
			for (let i = 0; i < moverCount; i++) {
				attract(movers[i], sun.sun, 1, 1)
			}
		}
	})
	...
	if (mappedValY) {
		...
		var pos = camera.position.clone().add(dir.multiplyScalar(distance))
		// mesh.position.copy(pos)
		// Sketch.sun.position.lerp(pos, 0.2)
		sun.sun.position.lerp(pos, 0.2)
		sun.sunAround.position.lerp(pos, 0.2)
	}
	for (let i = 0; i < moverCount; i++) {
		movers[i].update()
