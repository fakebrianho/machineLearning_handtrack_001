...
class Mover {
	...
	applyForce(force) {
		...
		let f = new THREE.Vector3().divideScalar(force, this.mass)
		this.acc.add(f)
	}
...
let test = new Mover(0, 0, 0, 15, 32, 16)
scene.add(test.mesh)
/*------------------------------
Attraction Function
------------------------------*/
const attract = (mover, attractor, amass, mmass) => {
	...
	let b = new THREE.Vector3(mover.position.x, mover.position.y, 0.5)
	let force = a.sub(b)
	let distanceSquared =
...
const animate = () => {
  ...
  ------------------------------*/
	...
	var vector = new THREE.Vector3(mappedVal, 1.0, 0.5)
	vector.unproject(camera)
	var dir = vector.sub(camera.position).normalize()
	var distance = -camera.position.z / dir.z
	var pos = camera.position.clone().add(dir.multiplyScalar(distance))
